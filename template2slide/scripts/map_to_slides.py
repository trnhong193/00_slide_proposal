...
class SlideMapper:
    ...
    def _map_system_requirements(self, sections: Dict[str, str]):
        ...
        # Group AI Training, AI Inference, Dashboard if they exist
        ...
        if ai_training or ai_inference or dashboard:
            content = []
            if ai_training:
                content.append({"level": 0, "text": "AI Training"})
                training_bullets = self._format_bullet_points(ai_training)
                content.extend(training_bullets)
            if ai_inference:
                content.append({"level": 0, "text": "AI Inference"})
                inference_bullets = self._format_bullet_points(ai_inference)
                content.extend(inference_bullets)
            if dashboard:
                content.append({"level": 0, "text": "Dashboard"})
                dashboard_bullets = self._format_bullet_points(dashboard)
                content.extend(dashboard_bullets)
    ...
    def _extract_timeline_milestones(self, content: str) -> List[Dict[str, Any]]:
        ...
        # Support both **Phase T0:** and **Phase T0**: (colon inside or outside bold)
        # Use DOTALL to match across newlines
        pattern1a = r'\*\*Phase\s+(T\d+):\*\*\s*(.+?)(?=\n\*\*Phase|\Z)'
        pattern1b = r'\*\*Phase\s+(T\d+)\*\*:\s*(.+?)(?=\n\*\*Phase|\Z)'
        matches1a = list(re.finditer(pattern1a, content, re.IGNORECASE | re.DOTALL))
        matches1b = list(re.finditer(pattern1b, content, re.IGNORECASE | re.DOTALL))
        matches1 = matches1a if matches1a else matches1b
